...
// ─── 0. Escrow initiated (buyer started checkout, payment not yet confirmed) ──

export async function sendEscrowInitiatedEmail(params: {
  sellerEmail: string;
  sellerName: string;
  buyerName: string;
  listingTitle: string;
  ...
  await dispatchEmail("escrow_initiated_seller", params.sellerEmail, params);
}
...
export async function sendEscrowFundedEmail(params: {
  ...
  amount: number;
  escrowId: string;
  // Deliberate business rule: buyer contact info is only ever shared once
  // payment is confirmed (funded), never at initiation. Optional — omitted
  // entirely from the email body if the buyer never set a phone number.
  buyerPhone?: string;
}): Promise<void> {
  await Promise.all([
